/* Space Invaders v10 – Endless, Highscore, Fullscreen, Pause, Sound, Delay-Fire */
...
const winOverlay      = document.getElementById('winOverlay');   // wird nie genutzt (Endless)
...
let keys, score, lives, gameOver;
let hitFlash, invincible, paused;
let highscore=0;
let musicPlaying=false;
let enemyCanFire=false;            // <-- neu: Schussfreigabe Gegner

/* === Highscore (localStorage) === */
const STORAGE_KEY='spaceinvaders_highscore';
const loadHighscore = ()=> highscore=parseInt(localStorage.getItem(STORAGE_KEY)||'0');
const saveHighscore = ()=>{ if(score>highscore){ highscore=score; localStorage.setItem(STORAGE_KEY,highscore);} };
...
function spawnWave(){
  ...
  enemyCanFire=false;
  setTimeout(()=>enemyCanFire=true,2000);
...
function updateHUD(){
  ...
  highLabel.textContent =`HI: ${highscore}`;
  levelLabel.textContent=`WAVE ${level}`;
}

/* === Drawing === */
const drawSprite=(sp,x,y)=>{ctx.fillStyle='#33ff33';
  sp.forEach((row,ry)=>[...row].forEach((b,cx)=>b==='1'&&ctx.fillRect(x+cx*PIXEL,y+ry*PIXEL,PIXEL,PIXEL)));};
...
function drawBullets(){ctx.fillStyle='#33ff33';
...
function fireBullet(){bullets.push({x:player.x+PLAYER_WIDTH/2-BULLET_WIDTH/2,y:player.y-BULLET_HEIGHT});
                      if(soundEnabled) laserPlayer().play(); }
function movePlayer(){if(keys['ArrowLeft'])player.x-=PLAYER_SPEED;
                      if(keys['ArrowRight'])player.x+=PLAYER_SPEED;
                      player.x=Math.max(0,Math.min(canvas.width-PLAYER_WIDTH,player.x));}
function moveBullets(){bullets.forEach(b=>b.y-=BULLET_SPEED);
...
function moveEnemies(){
  ...
  if(edge){enemies.speedX*=-1;
    enemies.forEach(e=>{e.y+=ENEMY_STEP_DOWN;
      if(e.alive&&e.y+ENEMY_HEIGHT>=player.y)gameOver=true;});}}
function enemyActions(){
  enemies.forEach(e=>{
    if(!e.alive) return;
    if(enemyCanFire && Math.random()<enemies.shootChance){          // <-- Check!
...
const handleBlink =()=>{if(invincible&&--hitFlash<=0)invincible=false;};
...
function gameLoop(){
  ...
  if(gameOver){saveHighscore(); gameOverOverlay.classList.remove('hidden'); return;}
  requestAnimationFrame(gameLoop);
}

/* === UI-Kontrollen === */
function togglePause(){paused=!paused; pauseBtn.textContent=paused?'play_arrow':'pause';
                       if(!paused)requestAnimationFrame(gameLoop);}
const enterFs =()=>!document.fullscreenElement&&document.documentElement.requestFullscreen();
const exitFs  =()=>document.fullscreenElement&&document.exitFullscreen();
function toggleSound(){
  soundEnabled=!soundEnabled;
  soundBtn.textContent=soundEnabled?'volume_off':'volume_up';  // durchgestrichen = an
}
function toggleMusic(){
  musicPlaying=!musicPlaying;
  musicBtn.textContent = musicPlaying?'music_off':'music_note';
  musicPlaying?bgMusic.play().catch(()=>{}):bgMusic.pause();
}
...
/* === Auto-Play Musik (falls Button schon aktiv) === */
window.addEventListener('load',()=>{ if(musicPlaying) bgMusic.play().catch(()=>{});});
